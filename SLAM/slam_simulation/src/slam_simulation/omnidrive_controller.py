...
class OmniWheelController:
    ...
    def update_target_pos(self):
        ...
        if t < self.t_prev + 1:
            dt = t - self.t_prev
            self.x = (1 - dt) * self.x_prev + dt * self.x_tgt
            self.y = (1 - dt) * self.y_prev + dt * self.y_tgt
            ...
            if len(self.cmd_buffer) > 0:
                dx, dy, da = self.cmd_buffer[0]
                self.cmd_buffer.popleft()

                self.x_tgt = self.x_prev + m.cos(self.ang_prev) * dx - m.sin(self.ang_prev) * dy
                self.y_tgt = self.y_prev + m.sin(self.ang_prev) * dx + m.cos(self.ang_prev) * dy

                if abs(dx) + abs(dy) + abs(da) > 1e-5:
                    self.x_tgt += np.random.normal(0., 0.03)
                    self.y_tgt += np.random.normal(0., 0.03)

                self.x_tgt = max(min(self.x_tgt, 1.), -1.)
                self.y_tgt = max(min(self.y_tgt, 1.5), -1.5)
    ...
    def __cmd_ros_sub(self, msg):
        self.cmd_buffer.append((msg.linear.x, msg.linear.y, msg.angular.z))
