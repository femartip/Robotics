...
class Beacons2DVisible:
    ...
    def start_ros(self) -> None:
        ...
        self.gazebo_sub = ros.Subscriber(self.gazebo_sub_name, ModelStates,
                                         callback=self.__gazebo_ros_sub,
                                         queue_size=self.QUEUE_SIZE)
        self.mannequin_pub = ros.Publisher(self.mannequin_pub_name, numpy_msg(Floats),
                                           queue_size=self.QUEUE_SIZE)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                turtle_x = self.model_states[self.turtle_key]['pose'].position.x
                turtle_y = self.model_states[self.turtle_key]['pose'].position.y
                quaternion = self.model_states[self.turtle_key]['pose'].orientation
                _, _, turtle_ang = euler_from_quaternion(
                    [quaternion.x, quaternion.y, quaternion.z, quaternion.w])

                # Get the aboslute locations of the mannequins
                mannequins = []
                mannequin_names = [key for key in self.model_states.keys() if "mannequin" in key]
                random.shuffle(mannequin_names)
                for mannequin in mannequin_names:
                    ...
                    man_dist = (man_x ** 2 + man_y ** 2) ** .5

                    man_ang = np.arctan2(man_y, man_x) - turtle_ang
                    man_ang = (man_ang + np.pi) % (2 * np.pi) - np.pi

                    if -0.567 < man_ang < 0.567:
                        mannequins.append(man_x + man_dist * random.gauss(0, self.NOISE_STDDEV))
                        mannequins.append(man_y + man_dist * random.gauss(0, self.NOISE_STDDEV))
    ...
    def __gazebo_ros_sub(self, msg):
        ...
        for ii, model in enumerate(msg.name):  # store models location
            ...
            if "turtlebot3" in model:
                self.turtle_key = model


def main():
    publisher = Beacons2DVisible()
    publisher.start_ros()
    publisher.run()
