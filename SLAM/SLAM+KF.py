...
class LocalSLAM:
    ...
    def run(self):
        ...
        # numpy logic. You can still view the vectors and matrices in the debugger after they have
        # been created.
        mu_t0 = np.zeros((11, 1))
        Sigma_t0 = np.identity(11) * 3.5 ** 2 # Certainty about initial beacon positions is low
        Sigma_t0[:3, :3] = np.identity(3) * 1e-6 # Certainty about initial robot position is high
    ...
    def kalman_motion_update(self, mu_old, Sigma_old, u_t, dt):
        ...
        # Motion noise on robot pose (No need to linearize here)
        R[:3, :3] = np.identity(3) * stdev ** 2
    ...
    def kalman_sensor_update(self, mu_bar, Sigma_bar, u_t, y_t, dt):
        """
        Kalman sensor update step. Instead of using known constants for beacon positions, now we
        only have estimates of their positions in mu_bar.
        ...
        :param u_t: contains the input command at time t (vector of length 2)
        ...
        # HERE THE SOLUTION
        # C = xt (11d) yt (8d)
        C = np.concatenate((np.concatenate((-np.identity(2),) * 4), # influence robot xy
                            np.zeros((8, 1)), # robot orientation (ignored)
                            np.identity(8)), axis=1) # influence beacon positions
        # Q-matrix is straightforward; no linearization and no off-diagonal correlations
        Q = np.identity(8) * 0.15 ** 2

        # Kalman sensor update formulas
